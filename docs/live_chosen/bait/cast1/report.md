# Auto-Research Report — Does gradient boosting beat a linear baseline on our noisy 7-feature tabular regression task?

_scenario: **live_research** · tick: 255 · report v18_

## Baseline (N3 — few-shot large model)
- status: `verified` · dev_metric: **0.352**

## Method (N4 — fine-tuned small model)
- status: `verified` · best dev_metric: **0.7603** (best ckpt @ step 60)

## Verdict
RESEARCH ANSWERED: fine-tuned model beats baseline (best_dev=0.760 >= 0.6041)

## Incidents (black box)
- `HUNG_RESTART` node=N4 action=bounce (tick 119)

